...
+ (NSDictionary *)metadataForURL:(NSURL *)url
{
	...
	if (!f.isNull())
	{
		...
		if (nil == image) {
			...
			//	image = [[[UIImage alloc] initWithContentsOfFile:imageCacheTag] autorelease];
			//} else {
				// Book-keeping...
				...
				NSArray *imageFileNames = [fileNames pathsMatchingExtensions: [NSArray arrayWithObjects: @"jpg", @"png",nil]];
                
                //NSArray *imageFileNames = [[NSArray arrayWithObjects: @"jpg", @"png",nil] retain];
				...
				while (fileName = [imageEnumerator nextObject]) {
					if ([TagLibMetadataReader isCoverFile:fileName]) {
                        ...
                        //image = [[[UIImage alloc] imageNamed:[path stringByAppendingPathComponent:fileName]] autorelease];
						//[image setName:imageCacheTag];
						break;
					}
				}
